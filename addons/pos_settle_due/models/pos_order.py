...
class PosOrder(models.Model):
    ...
    def _validate_orders_for_settle_due(self):
        ...
        orders = self.filtered(lambda order: order.state in ('paid', 'done') and order.customer_due_total > 0)
        if not orders:
            raise UserError(_('Only posted POS orders with remaining due can be settled.'))
