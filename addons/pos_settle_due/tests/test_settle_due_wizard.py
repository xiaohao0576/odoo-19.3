...
class TestSettleDueWizard(TestPoSCommon):
    ...
    def test_settle_due_multiple_orders_keeps_single_discount_and_line_notes(self):
        ...
        self.assertEqual(
            line_by_source[source_order_a.id].note,
            f'Settled source order: {source_order_a.name}',
        )
        self.assertEqual(
            line_by_source[source_order_b.id].note,
            f'Settled source order: {source_order_b.name}',
        )
